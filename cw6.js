...
function defaultTask(day) {
  ...
  <div class="task-btns row">
    ...
    <svg
      onclick="removingTask(this)" class="trash"
    ...
    <svg
      onclick="editTask(this)" class="modify"
      xmlns="http://www.w3.org/2000/svg"
      x="0px"
      y="0px"
      width="24"
...
function submitEdit(e) {
	...
	for (let i = 0; i < days[flagDay].task.length; i++) {
		// console.log(i)
		if (
			days[flagDay].task[i].taskId === globalTempId &&
			document.getElementById("edit-input").value
		) {
			days[flagDay].task[i].taskName =
				document.getElementById("edit-input").value;
			break;
		}
	}
...
function addTask(e) {
	if (
		...
		let newObj = {
			taskId: (
				Math.floor(Math.random() * (200000 - 90019 + 1)) + 90019
			).toString(),
			taskName: document.getElementById("add-new").value,
			taskTime: document
				.getElementById("add-new-time")
				.value.split(":")
				.join(""),
			isDone: false,
		};
